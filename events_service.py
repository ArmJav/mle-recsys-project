...
class EventStore:
    ...
    def load(self, path):
        ...
        logger.info(f"Loaded events data, shape={self._events_data.shape}")
    ...
    def get(self, user_id, k):
        ...
        """
        get_user = self._events_data[self._events_data['user_id'] == user_id]
        if len(get_user) > 0:
            user_events = list(get_user.groupby("item_id").apply(list).to_dict())
            user_events.reverse()
        else:
            return []
